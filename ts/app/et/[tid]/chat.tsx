...
export default function ({ tid, topic }: { tid: string, topic: PbTopic }) {
    ...
    const chatVisible = users.length > 1 || messages.length > 0;
    ...
    return (
        <div className="h-full md:p-2">
            ...
            <div className="flex md:border md:rounded border-t h-full overflow-hidden">
                {(chatVisible || sidePopup) && <div className="flex-1 h-full flex flex-col">
                    <div className="flex-1 relative overflow-y-auto">
                        {sidePopup && <div className="sticky w-full h-full top-0 flex flex-col bg-white p-2">
                            <SideContent users={users} topic={topic} tid={tid} chatVisible={chatVisible} />    
                        </div>}
                ...
                <div className={"flex flex-col border-l p-2 " + (chatVisible ? "max-md:hidden w-64" : "mx-auto mt-10 max-w-full border-l-0")}>
                    <SideContent users={users} topic={topic} tid={tid} chatVisible={chatVisible} />
                </div>
            </div>
        </div>
...
function SideContent({ users, topic, tid, chatVisible }: { users: UserView[], topic: PbTopic, tid: string, chatVisible: boolean }): React.ReactElement {
    const toast = useToast();
    ...
    return <>
        {chatVisible && <div className="border-b overflow-hidden flex flex-col">
